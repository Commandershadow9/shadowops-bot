...
| `mcp-watchdog` | http | `https://mcp.zerodox.de/.well-known/oauth-authorization-server` — Erreichbarkeit des ZERODOX MCP-Servers; NICHT `/mcp` (403/401 Normalzustand), sondern Discovery-Pfad über zweiten offenen Router; Anlass 2026-08-17: Ausfall 14:21, bemerkt 15:47 (86 min Blindflug); MCP hat Lesezugriff auf Kundendaten (seit 2026-08-17, PR #427) | 5 min | — |
| `mcp-drift-watchdog` | mcp-drift | MCP-Container läuft mit Code aus `HEAD` des lokalen `~/mcp`-Repos UND `/data/audit` beschreibbar — via `scripts/mcp-drift-watchdog.sh`; Anlass: Sicherheitspatch `a18d8c0` lag 7 Tage in `~/mcp` ohne Drift-Erkennung (seit 2026-08-17, PR #427) | 15 min | — |
...
| `seo-deep-audit-freshness` | pg-freshness | seo_agent-DB: letzter zerodox-Deep-Audit (mode='deep', status='completed') < 195h (8+ Tage) — unabhängige Schicht zu `seo-audit-freshness` (Daily, 49h); prüft ob wöchentlicher Deep-Audit tatsächlich durchläuft (Wochen-Strategie seit 2026-07-17) | 1 h | 8 min |
...
| `nightly-suite-watchdog` | nightly-suite | ZERODOX Web Quality Nightly — alarmiert ab `NIGHTLY_SCHWELLE` (default 3) aufeinanderfolgenden Laeufen ohne gruenen Abschluss; Zustandswechsel-Alert (nicht taeglich); meldet Status bei JEDEM Lauf an ZERODOX-Statusseite; via `scripts/nightly-suite-watchdog.sh` + `scripts/nightly-serie.sh`; Anlass ZERODOX#2467: 60 Laeufe ohne Erfolg, Dauerzustand als Einzelereignis unsichtbar (PR #426, seit 2026-08-19) | 6 h | — |
