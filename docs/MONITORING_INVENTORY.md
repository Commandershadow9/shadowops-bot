...
## 8 · Folge-Maßnahmen (2026-06-10, #277-Abschluss)

**Externer Uptime-Backstop (VPS-Totalausfall):** `.github/workflows/external-uptime.yml` — GitHub-Actions-Cron auf hosted ubuntu-latest (VPS-**unabhängig**), pingt zerodox.de/api/health + guildscout.eu/health alle 5 min, Discord-Alert in #🩺-uptime-alerts. **Ehrliche Alert-Klassifikation** (UNREACHABLE = Totalausfall-Verdacht / ERROR = 5xx / DEGRADED = 200+status≠ok). Webhook via Repo-Secret `UPTIME_DISCORD_WEBHOOK`. fail-closed real verifiziert (Negativ-Test: DNS-Fail→UNREACHABLE, 404→DEGRADED, Discord 204). Schedule ist Best-Effort (~5-15 min Drift) = nur Backstop; schnelle Erkennung machen die internen Watchdogs. **Operator-Folge (Defense-in-Depth):** UptimeRobot Free als zweiter unabhängiger Anbieter (HTTP-Monitor auf dieselben Endpoints, native Discord-Integration) — Account nötig, daher Operator-Aufgabe (Tracker ZERODOX#1294-Stil).

**Single-Source-Monitoring-Secrets (2-Orte-Pflege gelöst):** Bot lädt CRON_API_KEY/ZERODOX_AGENT_API_KEY/AKQUISE_AI_BEARER_TOKEN beim Start aus **ZERODOX/.env** (`src/utils/config.py:_load_monitoring_secrets`), nicht mehr als Duplikat in bot/.env → **Rotation = 1 Ort**. Pfad via `ZERODOX_ENV_PATH` (systemd-Drop-In `shadowops-bot.service.d/zerodox-env.conf`). **FAIL-CLOSED:** fehlt Quelle/Key → Self-Check feuert Discord-Alert „Secret-Quelle fehlt" (deckt deklarative Engine UND Legacy project_monitor.internal_health, der sonst still skippte). **⚠️ Bei Server-Umzug:** nur `ZERODOX_ENV_PATH` im Drop-In anpassen — sonst alarmieren alle ZERODOX-Checks laut (gewollt, kein stiller Verlust). GITHUB_PAT war unnötig in bot/.env kopiert (Bot nutzt GITHUB_TOKEN) → entfernt.
