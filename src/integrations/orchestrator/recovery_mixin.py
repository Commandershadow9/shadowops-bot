        ...
        try:
            ...
            if check.returncode != 0:
                ...
                if current_branch in ('main', 'master'):
                    logger.warning(f"⚠️ Direct push to {current_branch} blocked for security. Creating fix branch...")
                    fix_branch = f"fix/security-auto-{datetime.now().strftime('%Y%m%d-%H%M%S')}"
