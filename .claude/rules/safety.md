...
## Security-Agent-Team W1 (LIVE seit 2026-07-09, 7d-Soak läuft — #290 / PR #339; P1: PR #307)

> **W1-Betriebsstand (2026-07-09):** `SECURITY_TEAM_ENABLED=true` (Soak-Start), Units `security-orchestrator` + `security-npm-audit-worker` laufen `--user`, Secrets in `~/.config/shadowops-security-team.env` (600, Redis-Auth-URL Pflicht!). Crons: Trigger 05:23 / Reaper 06:41 / Soak-Compare 07:31 (`logs/security-soak-w1.log`). `security-freshness-watchdog` (pg-freshness `sec_jobs`, 26h) aktiv — bei Neuinstallation Timer ERST nach dem ersten erfolgreichen Trigger-Lauf enablen (Kaltstart-Alarm). Spec v2: `docs/design/2026-07-09-security-agent-team-v2-spec.md` (5 Wellen, Fix-Kanal = Claude-CLI + PR-Gate, Jules tot). Soak-Gate ~2026-07-16, bis dahin bleibt der Monolith Source-of-Truth für LLM-Scans.
- **Code:** `src/integrations/security_engine/team/` — `contracts.py` (SecurityJob/JobResult/JobStatus, pydantic v2), `base_worker.py` (BaseSecurityWorker: Lifecycle in `sec_jobs` + Exception-Isolation), `orchestrator.py` (Fan-out-Stub), `runner.py` (Redis-Subscribe + Graceful-Shutdown-Poll), `workers/npm_audit_worker.py`, `*_main.py`-Entrypoints. Spec: `docs/design/2026-06-02-security-agent-team-p1-spec.md`, Plan: `docs/plans/2026-06-02-security-agent-team-p1.md`.
- **Architektur-Entscheidung (fix):** Always-on systemd-Worker + Redis-Token-Cap (Cap-Enforcement erst P2, Seam via `SecurityJob.token_cost`). NICHT auf on-demand/Multiplexer umbauen ohne Brainstorm/Review.
- **Feature-Flag `SECURITY_TEAM_ENABLED`:** seit 2026-07-09 `true` (7d-Soak, Vergleich via `scripts/security-soak-compare.sh`). Bei Problemen im Soak: Flag in `~/.config/shadowops-security-team.env` auf `false` + Units stoppen = sauberer Rollback. Monolith `scan_agent.py` bleibt Source-of-Truth für LLM-Scans bis Cutover (W4/W5).
- **`SecurityDB.store_finding()` ist jetzt der GETEILTE Schreibpfad** — `deep_scan._store_finding` + `scan_agent` inline-INSERT delegieren darauf (verhaltenstreue Spalten-Obermenge, setzt `status='open'` + `found_at=NOW()`). NIEMALS wieder zu separaten inline-INSERTs aufspalten. **Dedup bleibt Caller-Sache** — der Helper macht KEIN Fingerprint-Lookup (scan_agent dedupt selbst, npm-Worker dedupt via `compute_finding_fingerprint`).
- **`sec_jobs` + `finding_fingerprint` entstehen via `SecurityDB._ensure_schema()`** — das Projekt hat KEINEN Migrations-Runner, `migrations/*.sql` ist nur Doku-Paritaet. Bei Schema-Aenderungen IMMER `_ensure_schema()` pflegen (nicht nur die .sql).
- **Neue Deps:** `pydantic>=2.0.0,<3.0.0` + `redis>=5.0.0,<8.0.0` in requirements.txt (redis war vorher nur lazy in `github_integration` importiert → nie gepinnt; der Top-Level-Import in `team/runner.py` macht ihn jetzt zur harten Dep). Nach Pull immer `pip install -r requirements.txt`.
- **systemd-Units NICHT per PR installieren** — `deploy/security-*.service` + `shadowops-security-team.env.example` sind Templates, Install = manueller Ops-Schritt (Symlink `~/.config/systemd/user/`, env-File chmod 600, `enable --now`), gleiche Regel wie bei Watchdogs. Worker laufen `--user` als `cmdshadow` (braucht Repo-Lese-Zugriff fuer `npm audit`).
- **npm-audit-Worker:** strukturierte npm-Fehler (z.B. ENOLOCK ohne `package-lock.json`, `{"error":...}` auf stdout) → `JobStatus.PARTIAL`, NICHT stilles `OK/0` (Vorfall im Review gefangen). npm exit-code ≠ 0 bei Vulns ist NORMAL → stdout trotzdem parsen, NICHT als Fehler werten.
- **W1 hat geliefert:** dauerhafter `sec:trigger`-Loop in `orchestrator_main` (Subscribe-Loop, Flag-Gate, Graceful Shutdown) + `trigger`-Durchreichung in `sec_jobs`. **W2 (offen, Spec v2 §8):** Result-Aggregation + NO_SUBSCRIBER-Detection + Token-Cap-Enforcement, `config-audit-worker` (7 Host-Regeln), `container-scan-worker` (Trivy-Konsum aus `/var/log/trivy-scans`).
