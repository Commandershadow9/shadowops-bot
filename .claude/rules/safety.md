...
## Security-Agent-Team P1 (seit 2026-06-02, #290 / PR #307)
...
- **Feature-Flag `SECURITY_TEAM_ENABLED` default OFF.** NIE ohne Soak (npm-audit-Output 7d gegen Monolith vergleichen) auf `true`. Monolith `scan_agent.py` bleibt Source-of-Truth bis Cutover (P3+).
...
- **P2 (offen):** Token-Bucket-Enforcement aktivieren, `code-scan-worker` (erster LLM-Worker), dauerhafter `sec:trigger`-Loop im `orchestrator_main` (P1 ist nur Flag-gated Stub), Result-Aggregation im Orchestrator.
