...
- `security_engine/team/` — Security-Agent-Team (P1, #290): `contracts.py` (SecurityJob/JobResult), `base_worker.py` (BaseSecurityWorker + Lifecycle/Exception-Isolation), `orchestrator.py` (Fan-out-Stub), `runner.py` (Redis-Subscribe-Entrypoint), `workers/npm_audit_worker.py` (+ `*_main.py` systemd-Entrypoints). Always-on systemd-Worker + Redis-Token-Cap (Cap-Enforcement erst P2). Feature-Flag `SECURITY_TEAM_ENABLED` default OFF, Monolith `scan_agent.py` bleibt Source-of-Truth. Spec: `docs/design/2026-06-02-security-agent-team-p1-spec.md`, Plan: `docs/plans/2026-06-02-security-agent-team-p1.md`.
