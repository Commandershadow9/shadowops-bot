...
- **Monitor-only Projekte aus Auto-Webhooks herausnehmbar**: Projekte koennen mit `auto_create_webhook: false` vom globalen Auto-Webhook-Setup ausgenommen werden. Das verhindert Repo-URL-Warnungen fuer technische Monitoring-Eintraege ohne eigenes GitHub-Repo.
- **Dash/Underscore-Lookup in Notifications normalisiert**: Patch-Notes-/Push-Notifications und Git-Tag-Versionsermittlung finden Config-Keys wie `mayday_sim` jetzt auch, wenn GitHub `mayday-sim` liefert. Damit degradieren Versionen nicht mehr still auf berechnete SemVer-Fallbacks.
...
- **`config.yaml` projects.mayday_sim.deploy.post_deploy_command**: Vereinfacht auf nur `sudo bash /srv/leitstelle/app/scripts/deploy.sh`. Vorheriger Versuch mit zusätzlichem `sudo -u keydev prisma generate` davor erzeugte EACCES (PR #460 Auto-Deploy fail), weil `deploy.sh` selbst als root läuft und in `web/src/generated/client/` root-owned Files erzeugt — der nachfolgende keydev-Call kann sie dann nicht überschreiben. Lösung: kein separates prisma generate vor deploy.sh, das Skript macht es seit [mayday-sim#457](https://github.com/Commandershadow9/mayday-sim/pull/457) selbst (als root) intern. Plus einmaliger `chown -R keydev:leitstelle /srv/leitstelle/app/web/src/generated` zur Aufräumung der Ownership-Drift vom manuellen Deploy heute morgen.
